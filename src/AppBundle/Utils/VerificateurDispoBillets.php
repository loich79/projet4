...
class VerificateurDispoBillets
{
    ...
    public function verifierDispoBillets($date)
    {
        ...
        if($nbBilletsReserves >= 1000) {
            $this->session->getFlashBag()->add('erreur', "Tous les billets pour cette date on été vendu veuillez selectionner une autre date !");
            return false;
        }
        ...
        if ($nbBilletsReserves >= 990 && $nbBilletsReserves < 1000) {
            $this->session->getFlashBag()->add('warningNbBillets', "Attention, il ne reste que quelques billets pour cette date, il est possible que la commande ne puisse être validé ! ");
        }
